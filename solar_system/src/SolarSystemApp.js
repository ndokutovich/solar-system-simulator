...
export class SolarSystemApp {
    ...
    /**
     * Toggle reflected light (Earthshine, etc.)
     */
    toggleReflectedLight(visible) {
        this.showReflectedLight = visible;

        if (visible && this.reflectedLights.size === 0) {
            // Create reflected lights for major bodies (planets with significant albedo)
            this.createReflectedLights();
        } else {
            // Toggle visibility of existing lights
            for (const [key, light] of this.reflectedLights) {
    ...
    createReflectedLights() {
        ...
        for (const [key, body] of this.bodies) {
            if (body.data && body.data.reflectedLight && body.data.reflectedLight.enabled) {
                ...
                const reflectedLight = new THREE.PointLight(
                    config.color,
                    config.intensity,
                    config.distance // Limited range
                );
                reflectedLight.visible = this.showReflectedLight;

                // Add to the body so it moves with it
